...
class Group_bo {
     ...
     * @return bool
     *
     * Metodo para validar os dados inentes ao processo de <i>add_edit</i> do controller <i>Group</i>.
     */
    public function validate_add_edit() {
        $status = TRUE;

        // Validando o campo <i>grnid</i> (ID do Grupo) se for informado.
        if(isset($this->data['grnid'])) {
            if(!is_numeric($this->data['grnid'])) {
