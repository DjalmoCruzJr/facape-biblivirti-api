...
define('EMAIL_MESSAGE_NEW_MEMBER', '<!doctype html><html lang="pt-br"><head><meta charset="UTF-8"><title>{email_smtp_user_alias}</title></head><body><h3>Olá, {uscnome}</h3><p><strong>{gradmin}</strong> acabou de adicioná-lo(a) como membro no grupo <strong>{grcnome}</strong>.</p><p>Obrigado(a) por utilizar os serviços da nossa plataforma!</p><p>Att,<br><strong>{email_smtp_user_alias}</strong><br><em><a href="mailto:{email_smtp_user}">{email_smtp_user}</a></em><br></p><p>Enviado em: <em>{sending_date}</em></p></body></html>');
...
define('EMAIL_KEY_GRADMIN', '{gradmin}');
