...
define('EMAIL_MESSAGE_NEW_MESSAGE', '<!doctype html><html lang="pt-br"><head><meta charset="UTF-8"><title>{email_smtp_user_alias}</title></head><body><h3>Olá, {uscnome}</h3><p>Sua mensagem foi envida para o grupo <strong>{grcnome}</strong> com sucesso!</p><p>Mensagem: <strong>{msctext}.</strong></p><p>Obrigado(a) por utilizar os serviços da nossa plataforma!</p><p>Att,<br><strong>{email_s/mtp_user_alias}</strong><br><em><a href="mailto:{email_smtp_user}">{email_smtp_user}</a></em><br></p><p>Enviado em: <em>{sending_date}</em></p></body></html>');
